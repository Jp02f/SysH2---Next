  ...
  return (
    <header className="w-full bg-white border-b border-zinc-100 px-8 py-1 lg:py-4 flex justify-between items-center shadow-sm">
      ...
      {usuario?.bloco && usuario?.apartamento && (
        <div className="hidden lg:flex items-center gap-2 text-[#4B0082]">
          <span className="text-xs font-bold uppercase">Bloco</span>
          <span className="text-2xl font-black">{usuario.bloco}</span>
          <span className="text-xs font-bold uppercase ml-4">Ap</span>
          <span className="text-2xl font-black">{usuario.apartamento}</span>
        </div>
      )}
